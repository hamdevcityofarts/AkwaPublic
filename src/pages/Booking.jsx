  ...
  return (
    <option key={room._id} value={room._id}>
      {room.name} — {isSelectedWithPromo
        ...
        {/* ✅ NOUVEAU : Bouton WhatsApp alternatif — à placer après le bouton submit */}
<div className="mt-4 flex items-center gap-3">
  <div style={{ flex: 1, height: 1, background: "#e5e7eb" }} />
  <span style={{ ...sans, fontSize: "10px", color: "#9ca3af", letterSpacing: "0.10em" }}>
    OU
  </span>
  <div style={{ flex: 1, height: 1, background: "#e5e7eb" }} />
</div>
 ...
 <a href={`https://wa.me/237XXXXXXXXX?text=${encodeURIComponent(
    `Bonjour, je souhaite finaliser ma réservation :\n` +
    `- Chambre : ${selectedRoom?.name || 'N/A'}\n` +
    `- Arrivée : ${form.checkin || 'N/A'}\n` +
    `- Départ : ${form.checkout || 'N/A'}\n` +
    `- Nom : ${form.surname} ${form.name}\n` +
    `- Email : ${form.email}`
  )}`}
  target="_blank"
  ...
  style={{
    ...
    cursor: "pointer",
    width: "100%",
    boxShadow: "0 3px 18px rgba(37,211,102,0.25)",
    display: "flex",
    alignItems: "center",
