...
const PaymentSuccess = () => {
  ...
  const fetchDetails = async (reservationId, paymentId) => {
    try {
      ...
      const resResponse = await fetch(`http://localhost:5000/api/reservations/${reservationId}`, {
      ...
      if (paymentId) {
        const payResponse = await fetch(`http://localhost:5000/api/payments/${paymentId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleSendEmail = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/reservations/${reservation._id}/send-confirmation`, {
        method: 'POST',
        headers: {
