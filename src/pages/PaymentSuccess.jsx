...
const PaymentSuccess = () => {
  ...
  const fetchDetails = async (reservationId, paymentId) => {
    try {
      ...
      const resResponse = await fetch(process.env.API_BASE_URL + `/reservations/${reservationId}`, {
      ...
      if (paymentId) {
        const payResponse = await fetch(process.env.API_BASE_URL + `/payments/${paymentId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleSendEmail = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(process.env.API_BASE_URL + `/reservations/${reservation._id}/send-confirmation`, {
        method: 'POST',
        headers: {
